   ...
   <nav-bread>
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li @click="editModalFlag=true">
                  <dl>
                    <dd><img src="/static/+.png" alt=""></dd>

                  </dl>
                  <div class="addr-opration addr-default">Add address</div>
                </li>
              </ul>
            </div>

            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;"@click="expand" v-bind:class="{'open':limit>3?true:false}">
                more
                <i class="i-up-down">
    ...
    <modal :mdShow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        ...
        <a href="javascirpt:;"class="btn btn--m"@click="isMdShow=false">取消</a>
      </div>
    </modal>
    <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':editModalFlag}">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">Edit Address</div>
          <button class="md-close"@click="editModalFlag=false">Close</button>
        </div>
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error error-show" v-if="errorTip"></span>
            </div>
            <ul>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input tabindex="1"name="loginname"placeholder="详细地址" v-model="addressed"class="regi_login_input">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPeople"></i>
                <input type="text"tabindex="2"v-model="yName"placeholder="您的姓名"name="password"class="regi_login_input">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon"></i>
                <input tabindex="2"v-model="phone"name="phone"placeholder="您的电话"class="regi_login_input"@keyup.enter="editAdd">
              </li>
            </ul>
          </div>
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login"@click="editAdd">添加</a>
          </div>
        </div>
      </div>
    </div>
    <div class="md-overlay"v-if="editModalFlag"@click="editModalFlag=false"></div>
  ...
  export default{
    ...
    methods:{
      ...
      editAdd(){
        ...
        let addressId=Math.floor(Math.random()*100000);
        let addAddress={
          addressId:addressId,
          userName:this.yName,
          streetName:this.addressed,
          postCode:100002,
          tel:this.phone,
          isDefault:false
        };
        if(addressId&&this.yName&&this.addressed&&this.phone) {
          axios.post("/users/addAddress", {addAddress}).then((response) => {
            let res = response.data;
            if (res.status == '0') {
              this.init();
            } else {
